...
class YOLOHead2(nn.Module):
    ...
    def __init__(self, in_channels=1024, S=7, B=2, C=20):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(inner_channels, inner_channels, kernel_size=3, stride=2, padding=1), # 14x14 → 7x7
            #nn.BatchNorm2d(1024),
            nn.LeakyReLU(negative_slope=0.1),
            ...
            nn.Linear(inner_channels * S * S, 4096),
            nn.LeakyReLU(negative_slope=0.1),
...
class YOLOv1(nn.Module):
    def __init__(self, S=7, B=2, C=20, pretrained=True):
        ...
        self.head = YOLOHead2(in_channels=1280,S=7,B=2,C=20)
...
class YoloLoss(nn.Module):
    ...
    def yolo_loss_vectorized(self, pred, target, device='cuda'):
        ...
        #wh_loss = torch.sum((torch.sqrt(pred_boxes_obj[...,3:5])- torch.sqrt(true_box_obj[...,3:5]))**2)
        wh_loss = torch.sum((torch.sign(pred_boxes_obj[..., 3:5]) * torch.sqrt(torch.abs(pred_boxes_obj[..., 3:5]) + 1E-6) 
                             - torch.sqrt(true_box_obj[..., 3:5] + 1E-6)) ** 2)
